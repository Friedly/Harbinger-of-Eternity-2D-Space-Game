...
const		bool										ety::CWorld::check_Limits						( const float& fLayer, const sf::Vector2f& c_sfVectorEntityPosition )
				...
				if( mp_c_etyCurrentGamesettings != NULL && mp_c_etyCurrentRessourceManager != NULL &&  mp_c_etyCurrentItemManagement != NULL )
				{
					...
					if( p_c_etyEntity.use_count() > 0 )
					{
						...
						if( p_c_etyTempItemModules != NULL )
						{
							...
							p_c_etyTempItemModules->set_WeaponItemToModule( "LEFT" ,mp_c_etyCurrentItemManagement->get_Item( "Turret"		) );
						}
					}
			...
			boost::weak_ptr<ety::CEntity>				ety::CWorld::create_NPCEntity					( const std::string& c_stdStringUniqueID, const std::string& c_stdStringEntityKey, const sf::Vector2f& c_sfEntityPosition, const float fAngle, const std::string& c_stdStringItemTemplate )
			{
				if( mp_c_etyCurrentGamesettings != NULL && mp_c_etyCurrentRessourceManager != NULL &&  mp_c_etyCurrentItemManagement != NULL )
				{
					boost::weak_ptr<ety::CEntity> p_c_etyEntity( create_SpaceshipEntity( c_stdStringUniqueID, c_stdStringEntityKey, c_sfEntityPosition, 0.f, fAngle ) );
					...
					if( p_c_etyEntity.use_count() > 0 )
					{
						...
						p_c_etyEntity.lock()->add_Component( new ety::CMovableNPCComponent() );

						ety::CItemModulesComponent* p_c_etyTempItemModules = p_c_etyEntity.lock()->get_Component<ety::CItemModulesComponent>("ItemModulesComponent");

						if( p_c_etyTempItemModules != NULL )
						{
							p_c_etyTempItemModules->set_EquipmentItemToModule( mp_c_etyCurrentItemManagement->get_Item( "LightEngine2"		) );
							p_c_etyTempItemModules->set_EquipmentItemToModule( mp_c_etyCurrentItemManagement->get_Item( "LightAmmoDump"		) );
							p_c_etyTempItemModules->set_EquipmentItemToModule( mp_c_etyCurrentItemManagement->get_Item( "LightArmor"		) );
							p_c_etyTempItemModules->set_EquipmentItemToModule( mp_c_etyCurrentItemManagement->get_Item( "LightGenerator"	) );
							p_c_etyTempItemModules->set_EquipmentItemToModule( mp_c_etyCurrentItemManagement->get_Item( "LightShield"		) );
					...
					m_mapTrigger["AIChaseRoutine"]->add_UnitEvent(UnitEventType::enNOTICESENEMYINRANGE, c_stdStringUniqueID, true);
			...
			boost::weak_ptr<ety::CEntity>				ety::CWorld::create_EnvironmentalEntity			( const std::string& c_stdStringUniqueID, const std::string& c_stdStringEntityKey, const sf::Vector2f& c_sfEntityPosition, const sf::Vector2f& c_sfVectorVelocity, const float fLayer, const float fAngle, const float fRotationSpeed )
			{
				sf::Vector2f c_sfEntityPosition2 = c_sfEntityPosition;

				//srand(time(NULL));
				int iRandomX = rand() % 850 +1;
				int iRandomY = rand() % 850 +1;

				if(rand() % 2 +1 == 2)
				{
					c_sfEntityPosition2.x += iRandomX;
					//srand(time(NULL));
					if(rand() % 2 +1 == 2)
					{
						c_sfEntityPosition2.y += iRandomY;
					}
				}
				else
				{
				...
				if( mp_c_etyCurrentGamesettings != NULL && mp_c_etyCurrentRessourceManager != NULL )
				{
					...
					if( p_c_etyEntity.use_count() > 0 )
					{
						...
						p_c_etyEntity.lock()->add_Component( new ety::CStaticRenderComponent( mp_c_etyCurrentRessourceManager->get_Texture( c_etyIniFile.read_String( c_stdStringEntityKey, "strEntityAppearance", "ERROR!" ) ), c_sfEntityPosition2, m_c_sfVectorLayerScaling, mp_c_etyCurrentGamesettings->get_Videomode(), true, fLayer, fAngle, false ) );
						p_c_etyEntity.lock()->add_Component( new ety::CCircleCollisionComponent() );
					...
					p_c_etyEntity.lock()->get_Component<ety::CAttributeComponent>("AttributeComponent")->m_c_etyAttributes.change_Attribute<sf::Vector2f>	( "c_sfVectorVelocity"	, new sf::Vector2f( c_sfVectorVelocity ), true );
			...
			void										ety::CWorld::register_ClassToLuabind			( ety::CLua* const p_c_etyLuaScript )
			{
				...
				[ 
					...
					.def( "get_TriggerByID"				, &CWorld::get_TriggerByID									),

					luabind::class_<ety::CTriggerEvent>("CTriggerEvent")
					.def( "set_Active"					, &ety::CTriggerEvent::set_Active			)
					.def( "get_Active"					, &ety::CTriggerEvent::get_Active			)
					.def( "get_TriggerEventType"		, &ety::CTriggerEvent::get_TriggerEventType	),
					...
					.def( "get_MoveableNPCComponent"	, &CEntity::get_Component<ety::CMovableNPCComponent>		),
					...
					.def( "continue_Movement"			, &ety::CMovableComponent::continue_Movement				),

					luabind::class_<ety::CMovableSpaceshipComponent, ety::CMovableComponent>("CMovableSpaceshipComponent")
					.def( "accelerate_Forward"			, &ety::CMovableSpaceshipComponent::accelerate_Forward		)
					.def( "accelerate_Backward"			, &ety::CMovableSpaceshipComponent::accelerate_Backward		)
					.def( "accelerate_Left"				, &ety::CMovableSpaceshipComponent::accelerate_Left			)
					.def( "accelerate_Right"			, &ety::CMovableSpaceshipComponent::accelerate_Right		)
					.def( "stop_Movement"				, &ety::CMovableSpaceshipComponent::stop_Movement			),

					luabind::class_<ety::CMovableNPCComponent, ety::CMovableComponent>("CMovableNPCComponent")
					.def( "add_PatrolPoint"				, &ety::CMovableNPCComponent::add_PatrolPoint				)
					.def( "chase_Target"				, &ety::CMovableNPCComponent::chase_Target					)
					.def( "start_Chase"					, &ety::CMovableNPCComponent::start_Chase					)
					.def( "stop_Patrol"					, &ety::CMovableNPCComponent::stop_Patrol					)
					.def( "stop_Chase"					, &ety::CMovableNPCComponent::stop_Chase					)
					.def( "set_PatrolModus"				, &ety::CMovableNPCComponent::set_PatrolModus				)		
					.def( "get_PatrolModus"				, &ety::CMovableNPCComponent::get_PatrolModus				),
...
const		sf::Vector2i&								ety::CWorld::get_CelestialButtonPosition		( void )
			...
			ety::CTrigger*					const		ety::CWorld::get_TriggerByID					( std::string strTriggerID )
			{
				return m_mapTrigger[strTriggerID];
			}
