...
void							ety::CDialog::draw_Dialogitems					( sf::RenderTexture* p_c_sfRenderTextureBuffer )
{
	for(std::list<CDialogitem*>::iterator itDialogitem = m_listDialogitems.begin(); itDialogitem != m_listDialogitems.end(); itDialogitem++)
	{
		(*itDialogitem)->draw_Dialogitem(p_c_sfRenderTextureBuffer);
	}

	if(mp_c_etyButtonTooltip != NULL)
	{
		CPictureControl*	p_c_etyPictureControlBackground	=	mp_c_etyButtonTooltip->get_TooltipBackground();
		CLabel*				p_c_etyLabelText				=	mp_c_etyButtonTooltip->get_TooltipLabel();

		if(p_c_etyPictureControlBackground != NULL && p_c_etyLabelText != NULL)
		{
			sf::Vector2f	c_sfVectorfPositionEdit		=	sf::Vector2f(CMouse::get_CustomCursorSprite().getGlobalBounds().width, CMouse::get_CustomCursorSprite().getGlobalBounds().height);
			sf::Vector2f	c_sfVector2fMousePosition	=	sf::Vector2f(float(CMouse::get_Position().x), float(CMouse::get_Position().y + c_sfVectorfPositionEdit.y));

			p_c_etyPictureControlBackground->set_RealPosition(c_sfVector2fMousePosition);
			p_c_etyLabelText->update_Dialogitem(0);

			p_c_etyPictureControlBackground->draw_Dialogitem(p_c_sfRenderTextureBuffer);
			p_c_etyLabelText->draw_Dialogitem(p_c_sfRenderTextureBuffer);
		}
...
ety::CDialogitem* const			ety::CDialog::update_Dialog						( const sf::Uint32& uiFrameTime )
{
	...
	sf::Vector2f	c_sfVector2fAnchor	=	get_Anchor(m_en_etyAnchor,true);
	m_c_sfVector2fRealPosition.x		= (c_sfVector2fAnchor.x) +  m_c_sfVector2fPosition.x;
	m_c_sfVector2fRealPosition.y		= (c_sfVector2fAnchor.y) +  m_c_sfVector2fPosition.y;
...
void							ety::CDialog::update_Language					( const std::string& strActualLanguage, ety::CLua* const p_c_luaScript )
{
	...
	while(1)
	{
		...
		if((*itDialogitems)->get_DialogitemType() == DialogitemType::enBUTTON)
		{
			...
			if(p_c_etyButton->get_TooltipLabel() != NULL)
			{
				p_c_etyButton->get_TooltipLabel()->set_Content(p_c_luaScript->get_TableString(p_c_etyButton->get_CustomID() + "Tooltip", strActualLanguage));
				p_c_etyButton->get_TooltipLabel()->update_Dialogitem(0);

				p_c_etyButton->get_TooltipBackground()->set_Width(float(p_c_etyButton->get_TooltipLabel()->get_Width() * 1.05));
				p_c_etyButton->get_TooltipBackground()->set_Height(float(p_c_etyButton->get_TooltipLabel()->get_Height() * 1.2));
			}
	...
	if(bCollectiveUpdate == true)
	{
		for(std::list<CDialogitem*>::iterator itDialogitem = m_listDialogitems.begin() ; itDialogitem != m_listDialogitems.end() ; itDialogitem++)
		{
			...
			else if((*itDialogitem)->get_DialogitemType() == DialogitemType::enPICTURECONTROL)
			{
				...
				p_c_etyDialogitemPictureControl->set_ColorFilter(c_etyColorBackgroundColors);
			}
...
void							ety::CDialog::set_TooltipButton					(CButton* const p_c_etyButtonTooltip)
{
	mp_c_etyButtonTooltip	=	p_c_etyButtonTooltip;
}
...
sf::Vector2f					ety::CDialog::get_Anchor						( Anchor::en_etyAnchor en_etyAnchor, const bool bAbsolute )
{
	...
	else if(en_etyAnchor == Anchor::enALIGNEDBOTTOMLEFT)
	{
		if(bAbsolute == true)
		{
			c_sfVector2fAnchor.y	=	fHeight - m_fHeight;
			c_sfVector2fAnchor.x	=	0.f;
		}
	}
	else if(en_etyAnchor == Anchor::enALIGNEDBOTTOMRIGHT)
	{
		if(bAbsolute == true)
		{
			c_sfVector2fAnchor.y	=	fHeight - m_fHeight;
			c_sfVector2fAnchor.x	=	fWidth - m_fWidth;
		}
	}
	else if(en_etyAnchor == Anchor::enALIGNEDTOPLEFT)
	{
		if(bAbsolute == true)
		{
			c_sfVector2fAnchor.y	=	0.f;
			c_sfVector2fAnchor.x	=	0.f;
		}
	}
	else if(en_etyAnchor == Anchor::enALIGNEDTOPRIGHT)
	{
		if(bAbsolute == true)
		{
			c_sfVector2fAnchor.y	=	0.f;
