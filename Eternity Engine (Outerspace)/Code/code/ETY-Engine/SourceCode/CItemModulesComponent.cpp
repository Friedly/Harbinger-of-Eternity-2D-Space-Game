...
const		bool													ety::CItemModulesComponent::create_EquipmentModule		( const sf::Vector2i& c_sfVectorClassRange, const ety::ModuleType::en_etyModuleType en_etyModuleType )
			{
				...
				m_c_stdMapEquipmentModuleByType[ en_etyModuleType ] = boost::shared_ptr<ety::CModule>( new ety::CModule( en_etyModuleType, c_sfVectorClassRange, sf::Vector2f( 0.f, 0.f ), this ) );
...
const		bool													ety::CItemModulesComponent::create_WeaponModule			( const std::string& c_stdStringModuleID, const sf::Vector2f& c_sfVectorPosition, const sf::Vector2i& c_sfVectorClassRange, const ety::ModuleType::en_etyModuleType en_etyModuleType )
			{
				if( en_etyModuleType == ety::ModuleType::enLASERWEAPON || en_etyModuleType == ety::ModuleType::enPROJECTILEWEAPON || en_etyModuleType == ety::ModuleType::enROCKETLAUNCHER )
				{
					...
					m_c_stdMapWeaponModuleByID[ c_stdStringModuleID ] = boost::shared_ptr<ety::CModule>( new ety::CModule( en_etyModuleType, c_sfVectorClassRange, c_sfVectorPosition, this ) );
			...
			void													ety::CItemModulesComponent::update_Component			( const sf::Uint32& uiFrameTime )
			{
				...
				for( itEquipmentModuleByType ; itEquipmentModuleByType != m_c_stdMapEquipmentModuleByType.end() ; itEquipmentModuleByType++ )
				{
					...
					ety::CItem* p_c_etyModuleItem = itEquipmentModuleByType->second->get_Item();

					if( p_c_etyModuleItem != NULL )
					{
						fCurrentMass += p_c_etyModuleItem->get_Mass();
					}
				}
				...
				for( itWeaponModuleByID ; itWeaponModuleByID != m_c_stdMapWeaponModuleByID.end() ; itWeaponModuleByID++ )
				{
					...
					ety::CItem* p_c_etyModuleItem = itWeaponModuleByID->second->get_Item();
				...
				{
					fCurrentMass += p_c_etyAttributeComponent->m_c_etyAttributes.get_AttributeAsObject<float>("fShipMass");

					p_c_etyAttributeComponent->m_c_etyAttributes.change_Attribute<float>( "fMass", fCurrentMass );
				}
			}
