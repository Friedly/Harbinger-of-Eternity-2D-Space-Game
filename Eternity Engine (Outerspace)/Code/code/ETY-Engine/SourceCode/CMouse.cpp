...
const sf::Vector2i		ety::CMouse::get_Position				( const sf::View& c_sfView )
{
	if( mp_c_sfRenderWindow != NULL )
	{
		return sf::Vector2i( int(mp_c_sfRenderWindow->convertCoords( ety::CMouse::get_Position(), c_sfView ).x), int(mp_c_sfRenderWindow->convertCoords( ety::CMouse::get_Position(), c_sfView ).y) );
	}
